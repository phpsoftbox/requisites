...
final class OrmEntityStorageAdapter implements MigratableStorageInterface
{
    ...
    private function validateFieldMap(): void
    {
        $properties = [
            $this->fieldMap->idProperty,
            $this->fieldMap->profileProperty,
            $this->fieldMap->selectorProperty,
            $this->fieldMap->schemaVersionProperty,
            $this->fieldMap->subjectTypeProperty,
            $this->fieldMap->subjectIdProperty,
            $this->fieldMap->payloadProperty,
            $this->fieldMap->attachmentsProperty,
            $this->fieldMap->createdAtProperty,
            $this->fieldMap->updatedAtProperty,
        ];

        foreach ($properties as $property) {
            if ($property !== null && !array_key_exists($property, $this->metadata->columns)) {
                throw new InvalidFieldMapException(sprintf(
                    'ORM requisites field map property "%s" is not present in metadata for "%s".',
                    $property,
                    $this->entityClass,
                ));
            }
    ...
    public function save(RequisitesRecord $record): void
    {
        ...
        $insertData      = $this->toDbData($record, includeCreatedAt: true);
        $insertException = null;
        try {
            $connection->transaction(static function (ConnectionInterface $transaction) use ($table, $insertData): void {
                $transaction
                    ->query()
        ...
        if ($updated === 0 && !$this->existsBySubjectAndProfile($connection, $table, $record)) {
            throw new StorageException('ORM requisites record disappeared during INSERT conflict recovery.', 0, $insertException);
        }
    }
    ...
    {
        if ($record->id !== null) {
            throw new StorageException('insertOrFind() accepts only transient ORM requisites records.');
        }
        ...
        try {
            $connection->transaction(static function (ConnectionInterface $transaction) use ($table, $insertData): void {
                $transaction->query()->insert($table, $insertData)->execute();
            });
        } catch (QueryException $exception) {
        ...
        if ($canonical === null) {
            throw new StorageException('ORM requisites record was not found after atomic create.');
        }
    ...
    public function migrationBatch(
        ...
        $connection     = $this->writeConnection();
        $table          = $connection->table($this->metadata->table);
        $idColumn       = $this->columnFor($this->fieldMap->idProperty);
        $profileColumn  = $this->columnFor($this->fieldMap->profileProperty);
        $selectorColumn = $this->columnFor($this->fieldMap->selectorProperty);
        $query          = $connection
            ->query()
            ->select(['*'])
            ->from($table)
        ...
        if ($afterId !== null) {
            $query->where($idColumn . ' > :after_id', ['after_id' => $afterId]);
        }
        if (is_string($selector) && $selector !== '') {
            $query->where($selectorColumn . ' = :selector', ['selector' => $selector]);
        }
        ...
        foreach ($query->fetchAll() as $row) {
            ...
            try {
                $records[] = $this->toRecord($this->mapper->hydrate($this->entityClass, $row));
            } catch (Throwable $exception) {
                throw new StorageException('Failed to hydrate ORM entity from requisites migration row.', 0, $exception);
            }
        }
    ...
    {
        if ($record->id === null) {
            throw new StorageException('Backfill migration requires a persisted ORM record id.');
        }

        $connection        = $this->writeConnection();
        $table             = $connection->table($this->metadata->table);
        $allData           = $this->toDbData($record, includeCreatedAt: false);
        $payloadColumn     = $this->columnFor($this->fieldMap->payloadProperty);
        $attachmentsColumn = $this->columnFor($this->fieldMap->attachmentsProperty);
        $versionColumn     = $this->columnFor($this->fieldMap->schemaVersionProperty);
        $data              = [
            $payloadColumn     => $allData[$payloadColumn],
            $attachmentsColumn => $allData[$attachmentsColumn],
            $versionColumn     => $record->schemaVersion,
        ];
        if ($this->fieldMap->updatedAtProperty !== null) {
            $updatedColumn        = $this->columnFor($this->fieldMap->updatedAtProperty);
            $data[$updatedColumn] = $allData[$updatedColumn];
        }
        ...
        $updated = $connection
            ...
            ->where($this->columnFor($this->fieldMap->idProperty) . ' = :id', ['id' => $record->id])
            ->where(
                $this->columnFor($this->fieldMap->schemaVersionProperty) . ' = :schema_version',
                ['schema_version' => $previousVersion],
            )
            ->execute();

        if ($updated !== 1) {
            throw new StorageException(sprintf(
                'Backfill update conflict for ORM record id=%s (expected version=%d).',
                (string) $record->id,
                $previousVersion,
            ));
        }
    ...
    {
        return new MigrationStorageInfo('orm', $this->resolvedConnectionName(), $this->metadata->table);
    }
    ...
    private function existsById(ConnectionInterface $connection, string $table, int|string $id): bool
    {
        return is_array($connection
            ->query()
            ->select([$this->columnFor($this->fieldMap->idProperty)])
            ->from($table)
            ->where($this->columnFor($this->fieldMap->idProperty) . ' = :id', ['id' => $id])
            ->limit(1)
            ->fetchOne());
    }
    ...
    private function findBySubjectAndProfile(
        ...
        try {
            return $this->toRecord($this->mapper->hydrate($this->entityClass, $row));
        } catch (Throwable $exception) {
            throw new StorageException('Failed to hydrate canonical ORM requisites record.', 0, $exception);
        }
    }
