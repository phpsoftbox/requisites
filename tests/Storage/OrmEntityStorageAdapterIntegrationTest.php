...
final class OrmEntityStorageAdapterIntegrationTest extends TestCase
{
    ...
    private static function runScenario(Database $database, string $driver): void
    {
        ...
        try {
            ...
            $atomicRecord = new RequisitesRecord(
                profile: 'company',
                selector: 'country:RU',
                schemaVersion: 1,
                subjectType: 'company',
            ...
            $inserted = $adapter->insertOrFind($atomicRecord);
            self::assertTrue($inserted->inserted);
            self::assertNotNull($inserted->record->id);

            $existing = $adapter->insertOrFind(new RequisitesRecord(
                profile: 'company',
                selector: 'country:KZ',
                schemaVersion: 1,
                subjectType: 'company',
            ...
            self::assertSame('country:RU', $existing->record->selector);
            self::assertSame(['name' => 'Atomic'], $existing->record->payload);
